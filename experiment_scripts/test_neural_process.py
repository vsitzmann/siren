...
coord_dataset_test = dataio.Implicit2DWrapper(img_dataset_test, sidelength=(32, 32), test_sparsity=10,
                                              generalization_mode='cnp_test')
image_resolution = (32, 32)

img_dataset_train = dataio.CelebA(split='train', downsampled=True)
coord_dataset_train = dataio.Implicit2DWrapper(img_dataset_train, sidelength=(32, 32), test_sparsity=10,
                                               generalization_mode='cnp_test')
...
model_input = {'coords': dataio.get_mgrid(image_resolution)[None, :].cuda(),
               'img_sub': coord_dataset_train[0][0]['img_sub'].unsqueeze(0).cuda(),
               'coords_sub': coord_dataset_train[0][0]['coords_sub'].unsqueeze(0).cuda()}
...
model_input_1 = {'coords': dataio.get_mgrid(image_resolution)[None, :].cuda(),
                 'img_sub': coord_dataset_train[idx1][0]['img_sub'].unsqueeze(0).cuda(),
                 'coords_sub': coord_dataset_train[idx1][0]['coords_sub'].unsqueeze(0).cuda()}
model_input_2 = {'coords': dataio.get_mgrid(image_resolution)[None, :].cuda(),
                 'img_sub': coord_dataset_train[idx2][0]['img_sub'].unsqueeze(0).cuda(),
                 'coords_sub': coord_dataset_train[idx2][0]['coords_sub'].unsqueeze(0).cuda()}
